...
#'
#' @export
get_activity_types <- function(semestry, timeout = 30) {
  endpoint <- "/v1/api/activitytypes"
  ...
  if (resp$status_code == 200) {
    ...
    stop(paste("Error: Failed to retrieve activity type data from the API. Status code:", httr::http_status(resp)$status_code))
  }
}
