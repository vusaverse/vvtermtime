...
#'
#' @return The retrieved booking type data from the API.
#'
#' @export
get_booking_types <- function(semestry, timeout = 30) {
  endpoint <- "/v1/api/bookingtypes"
  ...
  if (resp$status_code == 200) {
    ...
    stop(paste("Error: Failed to retrieve booking type data from the API. Status code:", httr::http_status(resp)$status_code))
  }
}
