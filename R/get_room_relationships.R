...
#'
#' @return The retrieved room relationship data from the API.
#'
#' @export
get_room_relationships <- function(semestry, timeout = 30) {
  endpoint <- "/v1/api/roomrelationships"
  ...
  if (resp$status_code == 200) {
    ...
    stop(paste("Error: Failed to retrieve room relationship data from the API. Status code:", httr::http_status(resp)$status_code))
  }
}
