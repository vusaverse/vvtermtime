...
#'
#' @export
get_activity_groups <- function(semestry, timeout = 30) {
  endpoint <- "/v1/api/activitygroups"
  ...
  if (resp$status_code == 200) {
    ...
    stop(paste("Error: Failed to retrieve activity group data from the API. Status code:", httr::http_status(resp)$status_code))
  }
}
