...
#'
#' @return The retrieved staff group data from the API.
#'
#' @export
get_staff_groups <- function(semestry, timeout = 30) {
  endpoint <- "/v1/api/staffgroups"
  ...
  if (resp$status_code == 200) {
    ...
    stop(paste("Error: Failed to retrieve staff group data from the API. Status code:", httr::http_status(resp)$status_code))
  }
}
