#' Retrieve a specific activity from the Semestry API
#'
#' This function sends a GET request to the activity endpoint of the Semestry API and retrieves data for a specific activity.
#'
#' @param semestry An authenticated Semestry object.
#' @param activity_code The activity code to retrieve data for.
...
#'
#' @export
get_activity <- function(semestry, activity_code, fields = NULL, format = NULL, timeout = 30) {
  endpoint <- paste0("/v1/api/activity/", activity_code)
  ...
  if (resp$status_code == 200) {
    ...
    stop(paste("Error: Failed to retrieve activity", activity_code, "from the API. Status code:", httr::http_status(resp)$status_code))
  }
}