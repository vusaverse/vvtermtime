...
#'
#' @return The retrieved supergroup data from the API.
#'
#' @export
get_supergroups <- function(semestry, timeout = 30) {
  endpoint <- "/v1/api/supergroups"
  ...
  if (resp$status_code == 200) {
    ...
    stop(paste("Error: Failed to retrieve supergroup data from the API. Status code:", httr::http_status(resp)$status_code))
  }
}
