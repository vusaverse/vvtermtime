#' Retrieve a specific course from the Semestry API
#'
#' This function sends a GET request to the course endpoint of the Semestry API and retrieves data for a specific course.
#'
...
#'
#' @export
get_course <- function(semestry, course_code, fields = NULL, format = NULL, timeout = 30) {
  endpoint <- paste0("/v1/api/course/", course_code)
  ...
  if (resp$status_code == 200) {
    ...
    stop(paste("Error: Failed to retrieve course", course_code, "from the API. Status code:", httr::http_status(resp)$status_code))
  }
}